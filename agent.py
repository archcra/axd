...
class Agent():
    ...
    def act(self, state, tau):
        ...
        nextState, _, _ ,_ = state.takeAction(action)
    ...
    def getAV(self, tau):
        ...
        pi = pi / (sum_pi * 1.0) # TODO 这个在以前的model中，np.sum(pi)有时为0，导致pi的每个元素，都是nan，如何破？
        # lg.logger_mcts.info('Now 4... edge=%s, tau=%s, pow=%s, pi=%s. ',edge.stats['N'],tau, pow(edge.stats['N'], 1/tau), pi)
